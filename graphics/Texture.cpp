...
namespace gfx {
   TextureRequest::TextureRequest(utl::StringView path, RepeatType repeat, FilterType filter)
   ...
   size_t TextureRequest::hash() const {
      ...
      h = (h << 5) + (h << 1) + (unsigned int)utl::stringViewHash(path);
      return h;
   }

   struct TextureBuffer {
      std::unique_ptr<utl::ColorRGBA[]> bits;
      utl::Int2 size;

      TextureBuffer() {}

      TextureBuffer(std::unique_ptr<utl::ColorRGBA[]> bits, utl::Int2 size)
   ...
   TextureBuffer loadPng(std::string const& textureFile) {
      ...
      if ((image_data = new utl::ColorRGBA[totalSize / 4]) == NULL) {
      ...
      TextureBuffer out = { std::unique_ptr<utl::ColorRGBA[]>(image_data), {(int)width, (int)height} };
      return std::move(out);
   }
   ...
   class TextureManagerPrivate : public ObjectPrivate{
      mutable std::unordered_map<TextureRequest, std::unique_ptr<Texture>, utl::ObjectHash<TextureRequest>> m_textures;
   public:
      ...
      DECLARE_GRAPHICS_PRIVATE(TextureManager)
   };

   TextureManager::TextureManager() :Object(new TextureManagerPrivate()) {}
   Texture *TextureManager::get(TextureRequest const &request) const{ return self()->get(request); }
